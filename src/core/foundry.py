...
class BuildResult:
    ...
    deploy_url: Optional[str] = None  # Live URL from Vercel
...
class BlackBox:
    ...
    def log(self, event: str, details: Optional[str] = None) -> None:
...
class Foundry:
    ...
    def build(self, manifest: GantryManifest, mission_id: str, deploy: bool = True) -> BuildResult:
        ...
        def _dead_mans_switch():
            ...
            if container:
                try:
                    console.print(f"[red][FOUNDRY] TIMEOUT! Killing Pod...[/red]")
                    container.kill()
        ...
        try:
            console.print(f"[cyan][FOUNDRY] Building: {project_name} (TTL: {BUILD_TIMEOUT_SECONDS}s)[/cyan]")
            ...
            try:
                ...
                if "Conflict" in str(e):
                    console.print(f"[yellow][FOUNDRY] Removing stale Pod...[/yellow]")
                    try:
            ...
            if has_requirements and manifest.stack == StackType.PYTHON:
                console.print(f"[cyan][FOUNDRY] Installing Python dependencies...[/cyan]")
                ...
                if dep_exit != 0:
                    dep_output_str = dep_output.decode("utf-8") if isinstance(dep_output, bytes) else str(dep_output)
                    console.print(f"[yellow][FOUNDRY] Dependency install warning: {dep_output_str[:100]}[/yellow]")
                    blackbox.log("DEPS_INSTALL_WARNING", dep_output_str[:200])
                else:
                    blackbox.log("DEPS_INSTALLED")
                    console.print(f"[green][FOUNDRY] Dependencies installed[/green]")
            ...
            if has_package_json and manifest.stack == StackType.NODE:
                console.print(f"[cyan][FOUNDRY] Installing Node dependencies...[/cyan]")
