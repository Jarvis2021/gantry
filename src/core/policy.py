...
class PolicyGate:
    ...
    def __init__(self, policy_path: Path = POLICY_PATH) -> None:
        ...
        console.print(
            f"[green][GATEKEEPER] Policy loaded: {len(self._config.forbidden_patterns)} forbidden patterns[/green]"
        )
    ...
    def _load_policy(self) -> PolicyConfig:
        ...
        if not self._policy_path.exists():
            console.print("[yellow][GATEKEEPER] Policy file not found, using defaults[/yellow]")
            return PolicyConfig(
    ...
    def _check_stack(self, manifest: GantryManifest) -> None:
        ...
        stack_value = (
            manifest.stack.value if hasattr(manifest.stack, "value") else str(manifest.stack)
        )

        if stack_value.lower() not in [s.lower() for s in self._config.allowed_stacks]:
            console.print(
                f"[red][GATEKEEPER] Access Denied: Stack '{stack_value}' not allowed[/red]"
            )
        ...
        for file_spec in manifest.files:
            for pattern in self._config.forbidden_patterns:
                if re.search(pattern, file_spec.content, re.IGNORECASE):
                    console.print(
                        f"[red][GATEKEEPER] Access Denied: Forbidden pattern in {file_spec.path}[/red]"
                    )
                    raise SecurityViolation(
