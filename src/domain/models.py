...
class GantryManifest(BaseModel):
    ...
    stack: StackType = Field(
        ...,
        description="Technology stack determining the base Docker image"
    )
    files: List[FileSpec] = Field(
        ...,
        min_length=1,
        description="Fabrication instructions: files to inject into the Pod"
    )
    audit_command: str = Field(
        ...,
        min_length=1,
        description="The Critic's test command (e.g., 'pytest', 'npm test')"
    )
