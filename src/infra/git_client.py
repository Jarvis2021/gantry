...
class GitProvider:
    ...
    def commit_and_push(self, branch: str = "main", message: str = "Gantry Auto-Deploy") -> str:
        ...
        """
        # For feature branches, we need special handling to share history with main
        if branch.startswith("feat/"):
            console.print("[cyan][GIT] Setting up feature branch workflow...[/cyan]")
            
            # Step 1: Create a README and push to main as the initial commit
            readme_path = self._workspace / "README.md"
            with open(readme_path, "w") as f:
                f.write("# Project\n\nAuto-generated by Gantry Fleet.\n")
            
            self._run(["git", "add", "README.md"])
            self._run(["git", "commit", "-m", "Initial commit: Add README"])
            self._run(["git", "branch", "-M", "main"])
            console.print("[cyan][GIT] Pushing initial main branch...[/cyan]")
            self._run(["git", "push", "-u", "origin", "main", "--force"])
            
            # Step 2: Create feature branch and add actual project files
